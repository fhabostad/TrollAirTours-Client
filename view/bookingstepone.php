...
/*
*if the variable 'd', the day of the month is before the 10th add a zero to prevent calendar crash
...
/*
*Checks which destination that is checked, runs through a foreach loop for that destination and prints 
*its enabled dates
*/
...
disableddates.push("9999-12-12"); //Adds a date to prevent calender crash.
 ...
 /*
 * Sets a variable equal to 'TimeList' and runs a for loop that removes its elements.
 * It clears the time drop down menu before selecting the destination.
 */
...
/*
* Calls the clearTimeDrop() function to clear time and checks which destination is checked,
* then sets it as the selected tour type.
* Gets the selected date and runs through a foreach loop that checks if the flight date
* is set to the selected date and the tour type is set to the selected tour type. 
* If that is true, the flight ID and departure time will be added to the TimeList.
* The function getPrice() is called after the destination, date and time is selected to
* ensure the right price is given.
*/
function timedrop()
 {
    clearTimeDrop();
...
/*
 * Clears the TimeList.
 * Checks the selected destination and displays the same form for Geiranger, Briksdalen and
 * Aakneset. If custom is checked, a predefined custom-form is displayed.
 */
function switchForm()
{
...
/*
 * Runs through a foreach loop to check if flight ID is set to selectedFlight,
 * if that is true, flightPrice is set to be FlightPrice and then set the Price
 * as flightPrice.
 */
function getPrice()
{
...
/*
 * Checks with foreach loop on foods, drinks and dutyfrees if they contain
 * a productID. If that is true the ProductPrice will be set to selected price for 
 * that category.
 * The total price is then set to zero and as long as a category's price is not undefined
 * it will be added to the total price.
 */
